...
def test_create_geolocation(client: TestClient):
    with patch(
        "app.utils.ipstack_client.get_geolocation_data",
        return_value=mock_ipstack_response,
    ):
...
def test_create_geolocation_existing(client: TestClient):
    with patch(
        ...
        response_create = client.post(
            "/api/v1/geolocations", json={"ip_or_url": "8.8.8.8"}
        )
        assert response_create.status_code == 201

        response_second_create = client.post(
            "/api/v1/geolocations", json={"ip_or_url": "8.8.8.8"}
        )
...
def test_create_geolocation_ipstack_error(client: TestClient):
    with patch(
        "core.crud.get_geolocation_data",
        side_effect=IPStackAPIError(detail="API Error"),
    ):
